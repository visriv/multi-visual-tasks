...
def get_kitti_info_path(
    ...
    data_idx_str = idx + file_tail
    prefix = pathlib.Path(prefix)
    if training:
        file_path = pathlib.Path('training') / info_type / data_idx_str
    else:
        file_path = pathlib.Path('testing') / info_type / data_idx_str
...
def check_kitti_directory(root_path):
...
def get_kitti_data_info(path, training=True,
                        label_info=True,
                        velodyne=False,
                        calib=False,
                        image_ids=7481,
...
def empty_result_anno():
    ...
    annotations.update({
        'name': np.array([]),
        'center_xyz': np.zeros([0, 3]),
        'size_lwh': np.zeros([0, 3]),
        'rot_theta': np.array([]),
        'score': np.array([]),
    })
    return annotations
...
def calculate_num_points_in_gt(data_path,
                                infos,
                                relative_path,
                                num_features=4):
    for info in infos:
...
def convert_cam_to_lidar_info(cam_info):
    res_info = {
        "data_idx": cam_info["image"]["image_idx"],
        "pc_path": cam_info["point_cloud"]['velodyne_path'],
    }

    if 'annos' in cam_info:
        annos = cam_info['annos']
        ...
        gt_boxes = box_camera_to_lidar(gt_boxes, calib["R0_rect"], calib["Tr_velo_to_cam"])

        # only center format is allowed. so we need to convert
        change_box3d_center(gt_boxes, [0.5, 0.5, 0], [0.5, 0.5, 0.5])

        res_info["annos"] = {
            'names': gt_names,
            'center_xyz': gt_boxes[:, 0:3],
            'size_lwh': gt_boxes[:, [4, 3, 5]],
            'rot_theta': -gt_boxes[:, 6],
