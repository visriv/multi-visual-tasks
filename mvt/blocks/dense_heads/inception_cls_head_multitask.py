...
class InceptionClsHeadMultitask(BaseClsDenseHead):
    ...
    def __init__(self,
                 ...
                 topk=(1, )):
        super(InceptionClsHeadMultitask, self).__init__(loss, topk)
        self.ce_loss = torch.nn.CrossEntropyLoss()
        self.aux_logits = aux_logits

        if aux_logits:
            self.AuxLogits = InceptionMAux(768, num_classes)
        self.group1 = nn.Sequential(
            OrderedDict([
                ('fc1', nn.Linear(2048, num_classes))
            ])
        )
        self.group2 = nn.Sequential(
            OrderedDict([
                ('fc2', nn.Linear(2048, 1)),
                ('sm2', nn.Sigmoid())
            ])
        )
    ...
    def forward(self, x):
        ...
        x = (self.group1(x[1]), self.group2(x[1]))
