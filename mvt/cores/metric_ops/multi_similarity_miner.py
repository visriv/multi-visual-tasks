...
from .cosine_distance import CosineSimilarity
...
class MultiSimilarityMiner(BaseTupleMiner):
    ...
    def mine(self, embeddings, labels, ref_emb, ref_labels):
        ...
        dtype = mat.dtype
        pos_ignore = (
            c_u.pos_inf(dtype) if self.distance.is_inverted else c_u.neg_inf(dtype)
        )
        neg_ignore = (
            c_u.neg_inf(dtype) if self.distance.is_inverted else c_u.pos_inf(dtype)
        )
