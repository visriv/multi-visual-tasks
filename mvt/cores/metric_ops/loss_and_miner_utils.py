...
def logsumexp(x, keep_mask=None, add_one=True, dim=1):
    if keep_mask is not None:
        x = x.masked_fill(~keep_mask, c_u.neg_inf(x.dtype))
    if add_one:
...
def get_random_triplet_indices(
    ...
    if len(a_idx) > 0:
        a_idx = c_u.to_device(torch.cat(a_idx), device=labels_device, dtype=torch.long)
        p_idx = c_u.to_device(torch.cat(p_idx), device=labels_device, dtype=torch.long)
        n_idx = c_u.to_device(torch.cat(n_idx), device=labels_device, dtype=torch.long)
    ...
    if indices_tuple is None:
        ...
        for i in range(len(labels)):
            ...
            if len(pos_idx) > 0 and len(neg_idx) > 0:
                ...
                a_idx = torch.ones_like(c_u.longest_list([p_idx, n_idx])) * i
                a_out.append(a_idx)
                p_out.append(p_idx)
                n_out.append(n_idx)
        try:
...
def convert_to_weights(indices_tuple, labels, dtype):
    ...
    counts = c_u.to_dtype(counts, dtype=dtype) / torch.sum(counts)
