...
from mtl.utils.io_util import imread, file_load, obj_dump
from mtl.utils.geometric_util import imresize
from mtl.utils.path_util import mkdir_or_exist


#----------------------------------------------------------------------------- 
# Laterly, we will extend single_device_test by select task, here only support 
# detection.
#-----------------------------------------------------------------------------
...
def single_device_cls_test(model, data_loader, show, out_dir):
    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            for i, (img, img_meta) in enumerate(zip(imgs, img_metas)):
                ...
                if out_dir:
                    out_file = osp.join(out_dir, img_meta['ori_filename'] + '.jpg')
                else:
                    out_file = None
                
                label_id = np.argmax(result[i])
                label_name = dataset.CLASSES[label_id]
                label_score = result[i][label_id]
                model.module.show_result(
                    img_show,
...
def multi_device_cls_test(model, data_loader, out_dir, gpu_collect):
    pass
...
def single_device_reg_test(model, data_loader, out_dir):
    pass


def multi_device_reg_test(model, data_loader, out_dir, gpu_collect):
    pass


def single_device_pos_test(model, data_loader, out_dir=None):
...
def multi_device_pos_test(model, data_loader, out_dir, gpu_collect):
    """Test model with multiple gpus.
...
def single_device_test(model, 
    ...
    if model_type == 'det':
        ...
        return single_device_cls_test(model, data_loader, show, out_dir)
    elif model_type == 'seg':
        return single_device_seg_test(model, data_loader, show, out_dir)
    elif model_type == 'reg':
        return single_device_reg_test(model, data_loader, out_dir)
    elif model_type == 'pos':
        return single_device_pos_test(model, data_loader, out_dir)
    else:
        return None


def multi_device_test(model, 
                      data_loader, 
                      model_type='det',
                      tmpdir=None, 
                      gpu_collect=False):
    if model_type == 'det':
        ...
        return multi_device_reg_test(model, data_loader, tmpdir, gpu_collect)
    elif model_type == 'pos':
        return multi_device_pos_test(model, data_loader, tmpdir, gpu_collect)
    else:
        return None
