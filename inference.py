...
def det_single_device_test(model, data_loader, score_thr=0.05):
    ...
    outputs = {"img_names": img_names, "img_ids": img_ids, "detections": results}
    return outputs
...
def save_det_json(data_dict, json_path):
    ...
    for i, det_result in enumerate(det_results):
        img_id = img_ids[i]
        img_info = {"file_name": img_names[i], "id": img_id}
...
def run_det_task(cfg_path, model_path, json_path, score_thr):
    ...
    outputs = det_single_device_test(model, data_loader, score_thr=score_thr)
...
def run_emb_task(
    ...
    outputs_ref = emb_single_device_test(model, data_loader_ref, with_label=True)
    ...
    outputs_qry = emb_single_device_test(model, data_loader_qry, with_label=False)

    print("(3/3) inferring query labels ...")
    outputs = infer_labels(outputs_qry, outputs_ref, label_mapping, top_k_list)

    save_submit_json(outputs, det_json_path, out_json_path, score_thr, top_k=1)
...
def run():
    ...
    run_det_task(
        str(det_cfg_path), str(det_model_path), str(det_json_path), det_score_thr
    )
