...
def single_device_test(model, data_loader):
    ...
    for _, data in enumerate(data_loader):
        ...
        if "bbox" in data:
            data["bbox"] = data["bbox"]
        bbox_id_batch = data["bbox_id"].cpu().numpy()
...
def parse_args():
    ...
    parser.add_argument(
        "--score-thr", type=float, default=0.05, help="score threshold (default: 0.05)"
    )
...
def main():
    ...
    for k in RANK_LIST:
        save_json(
            outputs, args.json_ori, args.json_out, score_thr=args.score_thr, top_k=k
        )
